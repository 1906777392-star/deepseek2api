...
function toIncognitoPayload(session) {
  const state = getSessionIncognitoState(session); const scope = session.role === "admin" ? "global" : "self";
...
async function handleAccountCreation(request, response, session) {
  ...
  try {
    const loginResult = body.token ? await bindDeepseekBearerToken({ token: body.token, deviceId, label: body.username })
      : body.smsCode ? await loginToDeepseekWithSms({ mobile: body.username, code: body.smsCode, deviceId })
...
async function handleSettingsRefresh(request, response, session, accountId) {
  ...
  if (!account) { sendError(response, 404, "Account not found"); return true; }
  const result = await reportClientSettingsForAccount(account);
  if (!result.ok) { sendError(response, 400, result.error || "Settings report failed"); return true; }
  sendJson(response, 200, { account: toPublicAccount(result.account), settingsIds: result.settingsIds });
  return true;
}
async function handleSmsCodeRequest(request, response) {
  const body = await readJsonRequest(request); const deviceId = resolveDeepseekDeviceId(body.deviceId);
  try { await sendDeepseekSmsCode({ mobile: body.username, deviceId }); sendJson(response, 200, { ok: true, deviceId }); }
  catch (error) { sendError(response, 400, error.message); }
  return true;
}
async function handleIncognitoUpdate(request, response, session) {
  const body = await readJsonRequest(request);
  if (session.role === "admin") setGlobalIncognitoEnabled(body.enabled); else setOwnerIncognitoEnabled(session.ownerId, body.enabled);
  sendJson(response, 200, { incognito: toIncognitoPayload(session) }); return true;
}
function handleAccountDeletion(response, session, url) {
  const account = resolveScopedAccount(session, url.pathname.split("/").pop());
  if (!account) { sendError(response, 404, "Account not found"); return true; }
  deleteAccountById(account.id); sendJson(response, 200, { accountId: account.id, ok: true }); return true;
}
async function handleCaptchaAction(request, response, session, url) {
  ...
  const [, accountId, action] = match; const account = resolveScopedAccount(session, accountId);
  if (!account) { sendError(response, 404, "Account not found"); return true; }
  try {
    if (action === "resolve") { const body = await readJsonRequest(request); sendJson(response, 200, { account: toPublicAccount(await resolveCaptchaManually(account, body)) }); return true; }
    if (action === "retry") { const result = await attemptCaptchaAutoSolveForAccount(account, { force: true }); sendJson(response, 200, { account: toPublicAccount(result.account), source: result.source }); return true; }
...
function resolveApiKeyAccount(session, requestedAccountId) {
  if (!isSharedAccountModeEnabled()) { const account = resolveScopedAccount(session, requestedAccountId); return isUsableAccount(account) ? account : null; }
  assertOwnerHasUsableAccount(session.ownerId); const accounts = listUsableAccountsForOwner(session.ownerId);
  return accounts.find((account) => account.id === (requestedAccountId ?? accounts[0]?.id)) ?? null;
}
export async function handlePrivateApiRequest({ request, response, session, url }) {
  ...
  const refreshMatch = /^\/api\/accounts\/([^/]+)\/settings\/refresh$/.exec(url.pathname);
  if (request.method === "POST" && refreshMatch) return handleSettingsRefresh(request, response, session, refreshMatch[1]);
  ...
  if (request.method === "POST" && url.pathname === "/api/api-keys") {
    const body = await readJsonRequest(request); let account;
    try { account = resolveApiKeyAccount(session, body.accountId); } catch (error) { sendError(response, 400, error.message); return true; }
    if (!account) { sendError(response, 404, "Account not found"); return true; }
    const result = createApiKeyRecord({ ownerId: session.ownerId, accountId: account.id, label: body.label || resolveAccountLabel(account), plainKey: body.plainKey || "", toolCallsEnabled: body.toolCallsEnabled, accountMode: body.accountMode });
    sendJson(response, 200, result); return true;
  }
  if (request.method === "PATCH" && url.pathname.startsWith("/api/api-keys/")) {
    const body = await readJsonRequest(request); const apiKey = updateApiKeyRecord(session.ownerId, url.pathname.split("/").pop(), { toolCallsEnabled: body.toolCallsEnabled, accountMode: body.accountMode });
    if (!apiKey) { sendError(response, 404, "API key not found"); return true; }
    sendJson(response, 200, { apiKey }); return true;
  }
