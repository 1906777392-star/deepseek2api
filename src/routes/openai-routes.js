...
function handleOpenAiError(response, error) { if (error.code === "USER_DISABLED" || error.code === "REQUEST_LIMIT") { sendError(response, resolveLimitStatus(error), error.message); return true; } if (error instanceof SyntaxError) { sendError(response, 400, "Invalid JSON body"); return true; } if (error.statusCode) { sendError(response, error.statusCode, error.message); return true; } return false; }
...
async function handleChatCompletionsRequest(request, response, apiKeyRecord) {
  await withOwnerRequestLimit(apiKeyRecord.ownerId, async () => {
    ...
    if (!account) {
      ...
      sendError(response, 404, "The API Key's selected DeepSeek account is unavailable. Rebind its token or switch this Key to account rotation.");
      return;
    }
...
export async function handleOpenAiRequest(request, response, url) {
  ...
  try {
    ...
    if (!handleOpenAiError(response, error)) throw error;
    return true;
  }
  return false;
}
